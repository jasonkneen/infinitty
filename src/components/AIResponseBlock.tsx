...
interface AIResponseBlockProps {
  ...
  onModelClick?: () => void  // Called when model name is clicked to change models
}

// Format raw model ID into a clean display name
function formatModelName(modelId: string | undefined): string {
  if (!modelId) return 'Unknown Model'

  // Handle provider-prefixed IDs like "openrouter/moonshotai/kimi-dev-72b:free"
  // Extract the last segment after the last /
  const segments = modelId.split('/')
  let name = segments[segments.length - 1] || modelId

  // Remove common suffixes
  name = name.replace(/:free$/i, '')
  name = name.replace(/-latest$/i, '')
  name = name.replace(/-preview$/i, ' Preview')

  // Clean up model name patterns
  // Handle patterns like "kimi-dev-72b" → "Kimi Dev 72B"
  // Handle patterns like "gpt-4o-mini" → "GPT-4o Mini"
  // Handle patterns like "claude-3-5-sonnet" → "Claude 3.5 Sonnet"

  // Replace hyphens with spaces for processing
  let formatted = name
    .replace(/-/g, ' ')
    // Fix version numbers like "3 5" back to "3.5"
    .replace(/(\d+)\s+(\d+)/g, '$1.$2')
    // Title case each word
    .split(' ')
    .map(word => {
      // Keep certain abbreviations uppercase
      const upperWords = ['gpt', 'ai', 'llm', 'api']
      if (upperWords.includes(word.toLowerCase())) {
        return word.toUpperCase()
      }
      // Keep size indicators uppercase
      if (/^\d+[bkm]$/i.test(word)) {
        return word.toUpperCase()
      }
      // Regular title case
...
export const AIResponseBlock = memo(function AIResponseBlock({ block, isFocused, onToolClick, showModelInfo = true, onModelClick }: AIResponseBlockProps) {
          ...
          {showModelInfo ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <span
                ...
                onMouseEnter={(e) => onModelClick && (e.currentTarget.style.color = theme.cyan)}
                onMouseLeave={(e) => onModelClick && (e.currentTarget.style.color = theme.foreground)}
                title={onModelClick ? 'Click to change model' : undefined}
              >
                {formatModelName(block.model)}
              </span>
            </div>
          ) : (
            <div />
