...
export function InteractiveBlock({ block, isExpanded, onToggleExpand, onExit, onDismiss, isFocused }: InteractiveBlockProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      webglAddon.onContextLoss(() => {
        // Guard against double-dispose (context loss can fire multiple times)
        if (webglAddonRef.current === webglAddon) {
          try {
            webglAddon.dispose()
          } catch (e) {
            console.warn('[InteractiveBlock] Error disposing WebGL addon on context loss:', e)
          }
  ...
  useEffect(() => {
    ...
    // Force refresh to apply changes (especially font changes)
  ...
  }, [settings.fontSize, settings.font.family, settings.cursorBlink, settings.cursorStyle, settings.lineHeight, settings.scrollback, xtermTheme])
