...
export function FontLoader() {
  useEffect(() => {
    ...
    if (fontsToLoad.size > 0) {
      // Each font needs its own weight specification
      const fontsList = Array.from(fontsToLoad)
        .map(font => `${font}:wght@400;500;600;700`)
        .join('&family=')
      const url = `https://fonts.googleapis.com/css2?family=${fontsList}&display=swap`
