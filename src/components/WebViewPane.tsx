...
export function WebViewPane({ pane }: WebViewPaneProps) {
  ...
  useEffect(() => {
    ...
    // Track if effect is still active (prevents updates after cleanup)
    let isActive = true
    ...
    const updateBounds = async () => {
      ...
      try {
        ...
        if (isActive) { // Only log if still active
    ...
    return () => {
      isActive = false // Mark as inactive to prevent stale updates
