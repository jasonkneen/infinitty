...
// Maximum number of persisted terminals to prevent unbounded memory growth
const MAX_PERSISTED_TERMINALS = 50

const terminalRegistry = new Map<string, TerminalInstance>()

// Evict oldest terminals when registry exceeds limit (LRU eviction)
function evictOldestTerminals(): void {
  if (terminalRegistry.size <= MAX_PERSISTED_TERMINALS) return

  // Sort by lastAccessed (oldest first)
  const entries = Array.from(terminalRegistry.entries())
    .sort((a, b) => a[1].lastAccessed - b[1].lastAccessed)

  // Evict oldest entries until we're under the limit
  const toEvict = entries.slice(0, terminalRegistry.size - MAX_PERSISTED_TERMINALS)
  for (const [key, instance] of toEvict) {
    instance.pty.kill()
    instance.canvasAddon?.dispose()
    instance.terminal.dispose()
    terminalRegistry.delete(key)
    console.log(`[Terminal] Evicted old terminal: ${key}`)
  }
}
...
export function useTerminal(containerRef: React.RefObject<HTMLDivElement | null>, options: UseTerminalOptions = {}) {
  ...
  const initTerminal = useCallback(async () => {
    ...
    if (persistKey && terminalRegistry.has(persistKey)) {
      ...
      if (terminalElement) {
        ...
        // Dispose and recreate CanvasAddon to ensure font changes are applied
        // The CanvasAddon caches font glyphs in a texture atlas that won't update otherwise
        console.log('[Terminal] Restoring terminal, recreating CanvasAddon for font settings')
        if (existing.canvasAddon) {
          try {
            existing.canvasAddon.dispose()
          } catch (e) {
            console.warn('[Terminal] Error disposing CanvasAddon on restore:', e)
          }
          existing.canvasAddon = null
        }

        requestAnimationFrame(() => {
          try {
            const newCanvasAddon = new CanvasAddon()
            term.loadAddon(newCanvasAddon)
            existing.canvasAddon = newCanvasAddon
            canvasAddonRef.current = newCanvasAddon
          } catch (error) {
            console.warn('[Terminal] Failed to recreate CanvasAddon on restore:', error)
          }
        ...
        canvasAddonRef.current = existing.canvasAddon
        setIsReady(true)
        return
      }
    ...
    // Store reference so we can dispose/recreate when font changes
    ...
    try {
      ...
      if (persistKey) {
        ...
        // Evict oldest terminals if we exceed the limit
        evictOldestTerminals()
      }
  ...
  useEffect(() => {
    const handleFontsLoaded = () => {
      ...
      console.log('[Terminal] Fonts loaded, recreating CanvasAddon')

      // Dispose old canvas addon and create new one to pick up newly loaded fonts
      if (canvasAddonRef.current) {
        try {
          canvasAddonRef.current.dispose()
        } catch (e) {
          console.warn('[Terminal] Error disposing CanvasAddon on font load:', e)
        }
      ...
      requestAnimationFrame(() => {
        try {
          ...
          console.warn('[Terminal] Failed to recreate CanvasAddon on font load:', error)
        }
  ...
  const prevFontWeightRef = useRef<string | undefined>(undefined)
  ...
  useEffect(() => {
    ...
    // Skip first render to avoid double CanvasAddon recreation
    // (initTerminal already creates the CanvasAddon with correct settings)
    if (!isInitializedRef.current) {
      isInitializedRef.current = true
      prevFontFamilyRef.current = options.fontFamily
      prevFontSizeRef.current = options.fontSize
      prevFontWeightRef.current = options.fontWeight
      prevThemeRef.current = options.theme
      return
    }
    ...
    const fontSizeChanged = options.fontSize !== undefined &&
      options.fontSize !== prevFontSizeRef.current
    const fontWeightChanged = options.fontWeight !== undefined &&
      options.fontWeight !== prevFontWeightRef.current
    ...
    if (options.fontWeight !== undefined) prevFontWeightRef.current = options.fontWeight
    ...
    // If font family, size, weight, or theme changed, dispose and recreate CanvasAddon
    // The CanvasAddon caches font glyphs in a texture atlas that won't update otherwise
    const needsCanvasReset = fontFamilyChanged || fontSizeChanged || fontWeightChanged || themeChanged
    if (needsCanvasReset) {
      console.log('[Terminal] Font/theme changed, recreating CanvasAddon', {
        fontFamilyChanged,
        fontSizeChanged,
        fontWeightChanged,
        themeChanged,
      ...
      // Dispose old canvas addon if it exists
      if (canvasAddonRef.current) {
        try {
          canvasAddonRef.current.dispose()
        } catch (e) {
          console.warn('[Terminal] Error disposing CanvasAddon:', e)
        }
        canvasAddonRef.current = null
      }

      // Create and load new canvas addon
      requestAnimationFrame(() => {
        try {
          const newCanvasAddon = new CanvasAddon()
          term.loadAddon(newCanvasAddon)
          ...
          console.warn('[Terminal] Failed to recreate CanvasAddon:', error)
        }

        // Refresh and fit after addon recreation
        requestAnimationFrame(() => {
          term.refresh(0, term.rows - 1)
