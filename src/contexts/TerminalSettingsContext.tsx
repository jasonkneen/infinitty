...
export function TerminalSettingsProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (ghosttyConfig.fontFamily) {
      // Try to find a matching predefined font
      const normalizedGhosttyFont = ghosttyConfig.fontFamily.toLowerCase().replace(/['"]/g, '')
      const matchingFont = TERMINAL_FONTS.find((f: TerminalFont) =>
        f.family.toLowerCase().includes(normalizedGhosttyFont) ||
        f.name.toLowerCase() === normalizedGhosttyFont
      )

      if (matchingFont) {
        updates.font = matchingFont
      } else {
        // Create a custom font entry for the Ghostty font
        updates.font = {
          id: 'ghostty-custom',
          name: ghosttyConfig.fontFamily,
          family: `"${ghosttyConfig.fontFamily}", monospace`,
          isMonospace: true,
          category: 'modern',
        }
      }
